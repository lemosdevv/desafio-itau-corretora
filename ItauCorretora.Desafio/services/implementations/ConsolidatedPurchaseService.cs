...
public class ConsolidatedPurchaseService : IConsolidatedPurchaseService
{
    ...
    public async Task<ConsolidatedPurchaseResult> ExecutePurchaseAsync(DateTime? referenceDate = null)
    {
        ...
        try
        {
            ...
            {
                var amount = totalAmount * (item.Weight / 100m); // item.Weight is a percentage (e.g., 30)
                if (!quotes.TryGetValue(item.StockId, out var price))
                {
                    _logger.LogWarning("No quote found for the asset. {StockCode}", item.Stock.Code);
                    continue;
                }
                var quantity = (int)(amount / price); // round down
                if (quantity > 0)
                {
            ...
            // 6. Check master custody balance
            ...
            // 7. Adjust quantities (subtract master balance)
            var finalPurchase = new List<OrderSummary>();
            foreach (var item in purchaseItems)
            {
                var stockId = item.Key;
                var (amount, quantity, price) = item.Value;
                if (masterCustodies.TryGetValue(stockId, out var masterStock))
                {
                    var quantityToBuy = Math.Max(0, quantity - masterStock.Quantity);
                    // Update the quantity to be purchased (discounting the balance)
                    // But in practice, the master balance will be used for distribution, so we don't need to buy less.
                    // The total quantity available for distribution will be quantity + masterStock.Quantity.
                    // So we keep the purchased quantity as quantity, and then distribute considering the balance.
                    // The important thing is that in the master purchase order, we only buy quantity.
                    // The master balance will be added to the quantity available for distribution.
            ...
            foreach (var order in finalPurchase)
            {
                var orderEntity = new Order
                {
                    ...
                    StockId = (await _context.Stocks.FirstOrDefaultAsync(s => s.Code == order.StockCode))?.Id ?? 0,
                    Date = DateTime.Now,
            ...
            foreach (var order in finalPurchase)
            {
                ...
                {
                    // Update quantity and average price (if necessary)
                    masterCustody.Quantity += order.Quantity;
                    masterCustody.AveragePrice = ((masterCustody.AveragePrice * (masterCustody.Quantity - order.Quantity)) + (order.Price * order.Quantity)) / masterCustody.Quantity;
                }
            }
            await _context.SaveChangesAsync();
            ...
            foreach (var customer in activeCustomers)
            {
                ...
                {
                    var stock = await _context.Stocks.FirstOrDefaultAsync(s => s.Code == order.StockCode);
                    var totalAvailable = order.Quantity + (masterCustodies.ContainsKey(stock.Id) ? masterCustodies[stock.Id].Quantity : 0);
                    ...
                    if (qtyForClient > 0)
                    {
                        ...
                        var movement = new AccountMovement
                        {
                            ...
                            StockId = stock.Id,
                            OrderId = null // we don't link it to a specific order
                        };
            ...
            // 12. Update master custody (subtract the distributed amount)
            foreach (var order in finalPurchase)
            {
                var stock = await _context.Stocks.FirstOrDefaultAsync(s => s.Code == order.StockCode);
