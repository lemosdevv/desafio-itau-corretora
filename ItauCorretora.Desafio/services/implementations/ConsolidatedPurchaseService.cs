...
public class ConsolidatedPurchaseService : IConsolidatedPurchaseService
{
    ...
    public async Task<ConsolidatedPurchaseResult> ExecutePurchaseAsync(DateTime? referenceDate = null)
    {
        ...
        try
        {
            ...
            // 5. Calculate quantities to purchase
            ...
            foreach (var item in activeWallet.Itens)
            {
                ...
                {
                    _logger.LogWarning("No quote found for asset {StockCode}. Skipping.", item.Stock.Code);
                    continue;
                }
                if (price <= 0)
                {
            ...
            foreach (var item in purchaseItems)
            {
                ...
                finalPurchase.Add(new OrderSummary
                {
                    ...
                    Details = SplitIntoLotesFracionario(quantity, stockCode)
                });
            }
            result.Orders = finalPurchase;

            // 8. Register master orders
            ...
            foreach (var customer in activeCustomers)
            {
                ...
                if (totalAmount == 0) continue; // safety
                ...
                foreach (var order in finalPurchase)
                {
                    ...
                    // Get master custody quantity for this stock
                    ...
                    if (qtyForClient > 0)
                    {
                        // Update or create client position
                        ...
                        else
                        {
                            ...
                            if (position.Quantity > 0)
                                ...
                                position.AveragePrice = order.Price; // fallback (shouldn't happen)
                        }
                        ...
                        // Debit movement in client account
            ...
            foreach (var dist in distributions)
            {
                foreach (var asset in dist.Assets)
                {
                    ...
                    var order = finalPurchase.FirstOrDefault(o => o.StockCode == asset.Ticker);
            ...
            // 12. Subtract distributed quantities from master custody
            foreach (var order in finalPurchase)
            {
                var stock = await _context.Stocks.FirstOrDefaultAsync(s => s.Code == order.StockCode);
                if (stock == null) continue;
    ...
    private bool IsPurchaseDay(DateTime date)
    {
        ...
        // Adjust for weekends: if day 6 is Monday and day 5 was Sunday, execute on Monday 6
